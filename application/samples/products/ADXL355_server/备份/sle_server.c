...
// --- Message Queue Configuration (NOW UNUSED, REPLACED BY FLAGS) ---
...
#define SLE_CONN_SUPERVISION_TIMEOUT_DEFAULT  0x1F4   // 500 * 10ms = 5s   
...
#define INITIAL_COLLECTION_START_COUNT      5
#define INITIAL_TRANSMISSION_START_COUNT    5
...
// --- Watchdog ---
#define WDT_SYSTEM_TIMEOUT_S            20  
...
#define SENSOR_DATA_BUFFER_SIZE       4096 // Buffer size in bytes
#define SENSOR_SAMPLE_SIZE_BYTES      (3 * sizeof(int16_t))
#define SENSOR_BUFFER_SAMPLES         (SENSOR_DATA_BUFFER_SIZE / SENSOR_SAMPLE_SIZE_BYTES) //4096 / 6 = 682 samples
/*============================================================================*/
...
// REFACTORED: Replaced message queues with volatile flags
...
static volatile bool g_send_data_flag = false;
...
static void print_memory_info(const char* tag) {
    ...
    if (LOS_MemInfoGet(m_aucSysMem0, &status) == LOS_OK) {
        osal_printk("[MEM-%s] Total=%u, Used=%u, Free=%u, MaxFree=%u\r\n",
            tag,
...
static uint16_t sle_set_adv_data(uint8_t *adv_data, uint16_t max_len)
{
    ...
    (void)memcpy_s(&adv_data[idx], max_len - idx, &adv_disc_level, len);
    idx += len;
    ...
    (void)memcpy_s(&adv_data[idx], max_len - idx, &adv_access_mode, len);
    idx += len;
...
static uint16_t sle_set_scan_response_data(uint8_t *scan_rsp_data, uint16_t max_len)
{
    ...
    (void)memcpy_s(scan_rsp_data, max_len, &tx_power_level, scan_rsp_data_len);
...
// REFACTORED: sensor_sampling_task now polls g_data_ready_flag
static void *sensor_sampling_task(void *arg) {
    (void)arg;
    // uint32_t dummy_msg;
    ...
    while (1) {
        if (!g_collection_phase_started) {
            osal_msleep(500); // Sleep if collection phase is not active
            continue;
        }

        // Check if the data ready flag is set
        if (g_data_ready_flag) {
            // Clear the flag immediately to indicate we are processing
            g_data_ready_flag = false; 

            // Double-check if collection was stopped in the meantime
...
// data_sending_task now polls g_send_data_flag
static void *data_sending_task(void *arg) {
    (void)arg;
    sample_at_log_print("%s Data Sending Task started.\r\n", SLE_SERVER_LOG);
    while (1) {
        // Check if the send data flag is set
        if (g_send_data_flag) {
            g_send_data_flag = false;

            g_transmission_counter++;
            dequeue_and_send_sensor_data();
            osal_printk("%s Trans Count: %u, Sample Count: %u\r\n", "[Count]", g_transmission_counter, g_sample_counter);
            g_sample_counter = 0; // Reset counter after sending data

            continue;

        } else {
            osal_msleep(20); // 20ms poll interval for commands
        }
    }
    return NULL;
}
...
void dequeue_and_send_sensor_data(void) {
    ...
    if (head != tail) {
        ...
        if (total_bytes_to_send > 0 && total_bytes_to_send <= SENSOR_DATA_BUFFER_SIZE) {
            if (tail > head) {
                (void)memcpy_s(tx_linear_buffer, sizeof(tx_linear_buffer), &g_sensor_data_buffer[head * 3], total_bytes_to_send);
            } else {
                uint32_t part1_bytes = (SENSOR_BUFFER_SAMPLES - head) * SENSOR_SAMPLE_SIZE_BYTES;
                (void)memcpy_s(tx_linear_buffer, sizeof(tx_linear_buffer), &g_sensor_data_buffer[head * 3], part1_bytes);

                uint32_t part2_bytes = total_bytes_to_send - part1_bytes;
                if (part2_bytes > 0) {
                    (void)memcpy_s(&tx_linear_buffer[part1_bytes], sizeof(tx_linear_buffer) - part1_bytes, g_sensor_data_buffer, part2_bytes);
                }
            }
        } else {
    ...
    osal_msleep(10);
    sle_uart_server_send_transmission_count((uint8_t)g_transmission_counter);
    osal_msleep(10);
    ...
    if (total_bytes_to_send > 0) {
        ...
        while (sent_bytes < total_bytes_to_send) {
            ...
            *p++ = (uint8_t)(timestamp >> 8);  *p++ = (uint8_t)(timestamp & 0xFF);
            ...
            (void)memcpy_s(p, sizeof(packet_buffer) - MTU_HEADER_SIZE, &tx_linear_buffer[sent_bytes], bytes_for_this_payload);
        ...
        *p = TRANSMISSION_END_FLAG;
        (void)sle_uart_server_send_report_by_handle(empty_packet, sizeof(empty_packet));
    }
    ...
    sle_acb_state_t conn_state, sle_pair_state_t pair_state, sle_disc_reason_t reason)
{
    ...
    if (conn_state == SLE_ACB_STATE_CONNECTED) {
        ...
        g_has_been_connected_once = true;
        g_collection_phase_started = false;
        g_is_operational = false;

        g_is_sending_data = false;

        // Clear flags on new connection
        g_data_ready_flag = false;
        g_send_data_flag = false;
...
static errcode_t sle_uart_server_add(void)
{
    ...
    (void)memcpy_s(app_uuid.uuid, app_uuid.len, g_sle_uuid_app_uuid, sizeof(g_sle_uuid_app_uuid));
...
static void *sle_uart_server_task(void *arg)
{
    ...
    sample_at_log_print("%s Using flags instead of message queues.\r\n", SLE_SERVER_LOG);
    ...
    while (1) {
        ...
        if (g_is_sle_connected) {
            (void)uapi_watchdog_kick();
            print_memory_info("HeartBeat_Connected");
        } else {
            if (!g_has_been_connected_once) {
                (void)uapi_watchdog_kick();
                print_memory_info("HeartBeat_Initial");
            } else {
                sample_at_log_print("%s Main task heartbeat. DISCONNECTED. Forcing WDT reboot...\r\n", SLE_SERVER_LOG);
